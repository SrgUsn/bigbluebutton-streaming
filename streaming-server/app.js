...
class BBBStreamService {
  ...
  async startStream(req, res) {
    try {
      ...
      const meetingInfo = await axios.get(constructUrl(this.bbb, 'getMeetingInfo', { meetingID: meetingId }));
      ...
      if (streamingContainers.length >= process.env.NUMBER_OF_CONCURRENT_STREAMINGS) {
        return res.status(500).json({ error: 'Слишком много потоков: Вы достигли лимита потоков. Попробуйте позже.' });
      }
